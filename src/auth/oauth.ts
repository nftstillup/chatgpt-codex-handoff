...
export interface OAuthDeps {
  ...
  logger: Logger;
  /** Override both discovery and grants for a restricted server. */
...
function pairingPage(opts: {
  ...
  const scopeLabels: Record<string, string> = {
    ...
    "handoff.read": "Read reviewed project summaries and your task results",
    "handoff.submit": "Submit work requests to the local queue (does not grant execution permissions)",
...
<div class="card">
  ...
  <p class="sub">ChatGPT is requesting access to workspace <strong>${escapedWorkspaceName}</strong>${opts.scopes.includes("handoff.submit") ? "" : " (read-only)"}:</p>
  <ul>${scopeList}</ul>
...
export function createOAuthRouter(deps: OAuthDeps): Router {
  ...
  router.get("/oauth/authorize", (req, res) => {
    ...
    const scopes = deps.supportedScopes
      ? (query.scope?.trim() ? query.scope.trim().split(/\s+/) : [...supportedScopes])
      : filterScopes(query.scope);
    if (scopes.some((scope) => !supportedScopes.includes(scope))) {
      fail("invalid_scope", "Unsupported scope for this server");
      return;
    }
